...
function onAttack_PassiveObject(passiveobj, damager, bounding_box, is_stuck) {
	...
	if (passiveobj) {
		...
		if (pChr) {
			...
			if (passiveobj.getPassiveObjectIndex() != 24104 && passiveobj.getPassiveObjectIndex() != 24105 && passiveobj.getPassiveObjectIndex() != 24106) procDevilStrikeGauge(pChr, passiveobj.getPassiveObjectIndex()); // 어벤저 state를 체크하여 충전이 필요한 상태라면 악마게이지를 충전시켜줍니다..
		}
	}
...
function isBattleMode() {
	...
	if (!isPvpMode && !isDungeonMode) { // 전투중이 아니라면
		return false;
	}
