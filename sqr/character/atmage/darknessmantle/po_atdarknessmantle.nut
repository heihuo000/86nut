...
function procCrackDarknessCircle(obj) // �տ��� ���� �� ��ü�� ��鸮�� ó��
{
	if(!obj)
		return;
		
	local circle_obj = obj.getVar().get_obj_vector(0); // ��
...
function setCustomData_po_ATDarknessMantle(obj, reciveData)
{
	...
	local suckVel = reciveData.readDword(); // 0. ������� ��
	// 0.������� ���� (<-100%->) 1.��Ʋ���ݷ�(%) 2.��ȭ���� 3.��ȭ���ӷ���
	local darkCircleRate = reciveData.readDword(); // ��Ҽ�Ŭ ���� (100%)
	local expAtkRate = reciveData.readDword(); // 1.��Ʋ���ݷ�(%)
	local slowLevel = reciveData.readDword(); // ��ȭ ����
	local slowTime = reciveData.readDword(); // ��ȭ ���ӽð�
	local sizeRate = reciveData.readDword(); // ũ�δ�Ŭ������ �߰� ��� ��Ҽ�Ŭ Ȯ���� (100%)
...
function procAppend_po_ATDarknessMantle(obj)
{
	...
	else if(passiveState == PASSIVEOBJ_SUB_STATE_3)
	{
		...
		{	
			...
			if(objListSize >= 4)
			{		
				local isEndAnimation = true; // �տ� �ִϸ��̼��� �������� üũ
