...
function checkCommandEnable_BlueDragonWill(obj)
{
	...
	{
		return obj.sq_IsCommandEnable(SKILL_BLUEDRAGONWILL); // �����忡���� Ư����ų�� ĵ���� �����մϴ�. �۾���:������ [2012.04.20] obj.sq_IsCommandEnable(SKILL_BROKENARROW);
	}
...
function onSetState_BlueDragonWill(obj, state, datas, isResetTimer)
{
	...
	if(substate == SUB_STATE_BLUEDRAGONWILL_0) {
		...
		//local chargeT = obj.sq_GetIntData(SKILL_BLUEDRAGONWILL, 0); // ���?�����ð�
		...
		if(targetObj) {
			...
			obj.getVar().push_vector(disX); // x�� �̵��Ÿ� : i: 4
			obj.getVar().push_vector(disY); // y�� �̵��Ÿ� : i: 5
		}
		else { // ����Ʈ �̵��Ÿ�
		...
		if(obj.isMyControlObject()) {
			...
			local spin_radius = obj.sq_GetIntData(SKILL_BLUEDRAGONWILL, 2); // ���� ���� (���� Ȯ����) (100~)
			local spin_r = spin_radius.tofloat() / 100.0;
			
			local firstAttackRate = obj.sq_GetBonusRateWithPassive(SKILL_BLUEDRAGONWILL, STATE_BLUEDRAGONWILL, 0, 1.0); //2.���ݷ�(%)
			local power = obj.sq_GetPowerWithPassive(SKILL_BLUEDRAGONWILL, STATE_BLUEDRAGONWILL, 1,-1,1.0);	 // �������ݷ�
			...
			sq_BinaryWriteDword(power); // �������ݷ�
			
			obj.sq_SendCreatePassiveObjectPacket(24246, 0, distanceL, -1, h); // �����̹��� ���� ����
			
			sq_BinaryStartWrite();
			sq_BinaryWriteDword(firstAttackRate); // ���ݷ�(%)
			
			obj.sq_SendCreatePassiveObjectPacket(24245, 0, distanceL, 5, h);
		}
	}
	else if(substate == SUB_STATE_BLUEDRAGONWILL_3) {
		// SUB_STATE_BLUEDRAGONWILL_3 ���꽺����Ʈ �۾�
	}
	else if(substate == SUB_STATE_BLUEDRAGONWILL_4) {
		// SUB_STATE_BLUEDRAGONWILL_4 ���꽺����Ʈ �۾�
	}
...
function prepareDraw_BlueDragonWill(obj)
{
	...
	if(substate == SUB_STATE_BLUEDRAGONWILL_0) {
		// SUB_STATE_BLUEDRAGONWILL_0 ���꽺����Ʈ �۾�
	}
	else if(substate == SUB_STATE_BLUEDRAGONWILL_1) {
		// SUB_STATE_BLUEDRAGONWILL_1 ���꽺����Ʈ �۾�
	}
	else if(substate == SUB_STATE_BLUEDRAGONWILL_2) {
		// SUB_STATE_BLUEDRAGONWILL_2 ���꽺����Ʈ �۾�
	}
	else if(substate == SUB_STATE_BLUEDRAGONWILL_3) {
		// SUB_STATE_BLUEDRAGONWILL_3 ���꽺����Ʈ �۾�
	}
...
function onProc_BlueDragonWill(obj)
{
	...
	if(substate == SUB_STATE_BLUEDRAGONWILL_0) {
    	...
    	local dis_x_len = sq_var.get_vector(4); // �� �̵��Ÿ�
		local dis_y_len = obj.getVar().get_vector(5); // y�� �̵��Ÿ�
		...
		if(sq_var.get_vector(0)) { // �������ӿ��� �̵��� �� ���� ������ �����ٸ�..
			if(sq_var.get_vector(1) != posY) { // �� posY�� ���غ��� �޶����ٸ�..
				sq_var.set_vector(0, 0); // �̵��÷��׸� off���ݴϴ�..
				sq_var.set_vector(1, posY);
			}
		}
		 
		if(obj.isMovablePos(dstX, dstY) && !sq_var.get_vector(0)) { // �̵��÷��׿� �̵����������� ���?�����ؾ� �̵�
			sq_setCurrentAxisPos(obj, 0, dstX);
			sq_setCurrentAxisPos(obj, 1, dstY);
		}
		else { // �̵��� �� ���� ������ ������..
			sq_var.set_vector(0,1); // �̵� �÷��� �ε��� 3 �̵��� �� ���� ������ ������ �� �׼��� ���̻� �̵����Ѵ�..
			...
			if(offset != 0) {				
				...
				local totalLen = sq_var.get_vector(4); // ���̵��Ÿ�
...
function onEndCurrentAni_BlueDragonWill(obj)
{
	...
	if(substate == SUB_STATE_BLUEDRAGONWILL_0) {
		...
		//else {
			...
			//obj.sq_IntVectPush(-1); // 
		//}
		local distance = obj.sq_GetIntData(SKILL_BLUEDRAGONWILL, SKL_STATIC_INT_IDX_3); // ������ �Ÿ�
