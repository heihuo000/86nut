function setCustomData_po_bladespirit_skill(obj, receiveData)
{
	...
	{
		case 0:
			local skillLevel = sq_GetSkillLevel(parentChr, 103);
			local SizePx = sq_GetLevelData(parentChr, 103, 0, skillLevel);
			local Time = sq_GetLevelData(parentChr, 103, 1, skillLevel);
			local Ms_Count = sq_GetLevelData(parentChr, 103, 2, skillLevel);
		...
		case 1:
			...
			local damage = parentChr.sq_GetPowerWithPassive(103, -1, 3, -1, 1.0);
			local attackInfo = sq_GetCurrentAttackInfo(obj);
			sq_SetCurrentAttackPower(attackInfo, damage);
			local skillLevel = sq_GetSkillLevel(parentChr, 103);
			local Rate = sq_GetLevelData(parentChr, 103, 5, skillLevel);
			Rate = Rate * 0.1;
			local Level = sq_GetLevelData(parentChr, 103, 4, skillLevel);
			local Time = sq_GetLevelData(parentChr, 103, 6, skillLevel);
			local Vauel = sq_GetLevelData(parentChr, 103, 7, skillLevel);
